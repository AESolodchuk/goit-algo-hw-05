...
first_text = requests.get(
    "https://drive.google.com/uc?id=18_R5vEQ3eDuy2VdV3K5Lu-R-B-adxXZh"
).text
second_text = requests.get(
...
te_search_boyer_moore_first_text = timeit.timeit(
    lambda: boyer_moore.boyer_moore_search(first_text, phraze_to_find_first_text),
    number=5,
)
...
te_search_rabin_karp_first_text = timeit.timeit(
    lambda: rabin_karp.rabin_karp_search(first_text, phraze_to_find_first_text),
    number=5,
)
...
ti_search_rabin_karp_first_text = timeit.timeit(
    lambda: rabin_karp.rabin_karp_search(first_text, phrazae_to_find_improvization),
    number=5,
)
...
te_search_rabin_karp_second_text = timeit.timeit(
    lambda: rabin_karp.rabin_karp_search(second_text, phraze_to_find_second_text),
    number=5,
)
